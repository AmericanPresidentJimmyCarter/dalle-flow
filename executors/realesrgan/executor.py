...
class RealESRGANUpscaler(Executor):
    # resrgan_models is organized as:
    # {
    #   model_name: {
    #     'name': str,
    ...
    def __init__(self,
        ...
        **kwargs,
        ...
        for model_name in models_to_load:
            ...
            if model_type == RESRGAN_MODELS.RealESRGAN_x4plus_anime_6B:  # x4 RRDBNet model with 6 blocks
            ...
            if model_type == RESRGAN_MODELS.RealESR_animevideov3:  # x4 VGG-style model (XS size)
                model = SRVGGNetCompact(num_in_ch=3, num_out_ch=3, num_feat=64, num_conv=16, upscale=4, act_type='prelu')
                netscale = 4
                file_url = ['https://github.com/xinntao/Real-ESRGAN/releases/download/v0.2.5.0/realesr-animevideov3.pth']
            if model_type == RESRGAN_MODELS.RealESR_general_x4v3:  # x4 VGG-style model (S size)
    ...
    def realesrgan(self, docs: DocumentArray, parameters: Dict, **kwargs):
        ...
        model_name = parameters.get('model_name',
            list(self.resrgan_models.values())[0]['name'])

        for doc in docs:
            img = document_to_pil(doc)
            img_arr = np.asarray(img)

            model_dict = self.resrgan_models.get(model_name, None)
            if model_dict is None:
                raise ValueError(
                    f'Unknown RealESRGAN upscaler specified: {model_name}')
